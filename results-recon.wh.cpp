// ==WindhawkMod==
// @id              results-recon
// @name            Search results recon (temporary)
// @description     Logs every element the search page builds as you type, to map the results area. Changes nothing.
// @version         0.1
// @author          bardelyne
...
Output goes to `%TEMP%\results-recon.log`.
*/
// ==/WindhawkModReadme==
...
// Append a line to %TEMP%\results-recon.log. The host owns the debugger channel
...
void Rec(const wchar_t* fmt, ...) {
    ...
    wcscat_s(path, MAX_PATH, L"results-recon.log");
...
class VisualTreeWatcher
   ...
   private:
    HRESULT STDMETHODCALLTYPE OnVisualTreeChange(ParentChildRelation,
        ...
        // Log every element as it is created, not just the first of each
        // type: the results list is built while the user types, so the
        // interesting structure only exists mid-session.
        ...
        // Anything plausibly part of the results surface is worth extra
        // detail: a list, a repeater, a web view, or a named container.
        bool interesting =
            wcsstr(type.c_str(), L"ListView") || wcsstr(type.c_str(), L"ItemsRepeater") ||
            wcsstr(type.c_str(), L"ListViewItem") || wcsstr(type.c_str(), L"WebView") ||
            wcsstr(type.c_str(), L"ScrollViewer") || wcsstr(type.c_str(), L"ItemsControl") ||
            wcsstr(type.c_str(), L"SearchResult") || wcsstr(type.c_str(), L"Result") ||
            !name.empty();

        if (interesting) {
            Rec(L"ADD  %-58ls %ls%ls   (%.0fx%.0f)", type.c_str(),
                name.empty() ? L"" : L"#", name.c_str(), w, h);
        } else {
            Rec(L"add  %ls", type.c_str());
        }
...
void Wh_ModAfterInit() {
    Wh_Log(L">");
    Rec(L"=== results recon attached, pid=%lu ===", GetCurrentProcessId());
