#!/bin/sh
# Compiles and installs everything-search.wh.cpp the way Windhawk's own
...
NAME="local@everything-search_0.1_$(date +%H%M%S).dll"
...
"$CXX" --target=x86_64-w64-mingw32 -shared -O2 -std=c++23 \
  ...
  '-DWH_MOD_ID=L"local@everything-search"' '-DWH_MOD_VERSION=L"0.1"' \
  -include windhawk_api.h -I"$INC" \
  -L"$ENGDIR" \
  -Wl,--export-all-symbols \
  -o "$MODS/$NAME" \
  -x c++ everything-search.wh.cpp \
  -lwindhawk -lole32 -loleaut32 -lruntimeobject

cp everything-search.wh.cpp "$SRCDIR/local@everything-search.wh.cpp"

powershell.exe -NoProfile -Command "
  \$k = 'HKLM:\SOFTWARE\Windhawk\Engine\Mods\local@everything-search';
  if (-not (Test-Path \$k)) { New-Item -Path \$k | Out-Null }
  \$s = 'HKLM:\SOFTWARE\Windhawk\Engine\Mods\local@everything-search\Settings';
  if (-not (Test-Path \$s)) { New-Item -Path \$s | Out-Null }
  Set-ItemProperty \$k -Name 'LibraryFileName' -Value '$NAME';
  Set-ItemProperty \$k -Name 'Include' -Value 'SearchHost.exe';
