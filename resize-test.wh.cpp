// ==WindhawkMod==
// @id              resize-test
// @name            Search split test (temporary)
// @description     Shrinks the results WebView and puts a panel beside it, to test the two-pane layout.
// @version         0.1
// @author          bardelyne
...
Output goes to `%TEMP%\resize-test.log`.
*/
// ==/WindhawkModReadme==
...
// Append a line to %TEMP%\resize-test.log. The host owns the debugger channel
...
void Rec(const wchar_t* fmt, ...) {
    ...
    wcscat_s(path, MAX_PATH, L"resize-test.log");
   ...
   private:
    HRESULT STDMETHODCALLTYPE OnVisualTreeChange(ParentChildRelation,
        ...
        try {
            ...
            panel.Margin(wux::ThicknessHelper::FromLengths(0, 96, 12, 12));
...
void Wh_ModAfterInit() {
    Wh_Log(L">");
    Rec(L"=== split test attached, pid=%lu ===", GetCurrentProcessId());
