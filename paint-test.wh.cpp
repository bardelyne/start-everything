// ==WindhawkMod==
// @id              paint-test
// @name            Search paint test (temporary)
// @description     Tries to add one visible element to the search page, to prove XAML injection works inside the sandbox.
// @version         0.1
// @author          bardelyne
...
Output goes to `%TEMP%\paint-test.log`.
*/
// ==/WindhawkModReadme==
...
// Append a line to %TEMP%\paint-test.log. The host owns the debugger channel
...
void Rec(const wchar_t* fmt, ...) {
    ...
    wcscat_s(path, MAX_PATH, L"paint-test.log");
...
class VisualTreeWatcher
   ...
   private:
    HRESULT STDMETHODCALLTYPE OnVisualTreeChange(ParentChildRelation,
        ...
        // The search page root. Everything we add hangs off its grid.
        if (wcscmp(element.Type, L"Cortana.UI.Views.TaskbarSearchPage") != 0) {
            return S_OK;
        }
        Rec(L"TaskbarSearchPage added - attempting to inject a visible element");
        ...
        if (!page) {
            Rec(L"FAIL: page is not a FrameworkElement");
            return S_OK;
        }
        g_xamlThreadId.store(GetCurrentThreadId());

        // The page's own RootGrid is the anchor seen in the recon dump.
        auto rootObj = FindDescendantByName(page, L"RootGrid", 8);
        auto root = rootObj ? rootObj.try_as<wuxc::Grid>() : nullptr;
        if (!root) {
            Rec(L"FAIL: could not find Grid#RootGrid under the page");
            return S_OK;
        }
        Rec(L"found Grid#RootGrid");

        if (FindDescendantByName(root, L"WindhawkPaintTest", 4)) {
            Rec(L"already injected, skipping");
            return S_OK;
        }

        // Construct and attach. This is the step that has never been tested
        // inside an AppContainer -- reading the tree is not the same as
        // building XAML objects in it.
        try {
            wuxc::Border box;
            box.Name(L"WindhawkPaintTest");
            box.Width(280);
            box.Height(180);
            box.HorizontalAlignment(wux::HorizontalAlignment::Right);
            box.VerticalAlignment(wux::VerticalAlignment::Top);
            box.Margin(wux::ThicknessHelper::FromLengths(0, 24, 24, 0));
            box.CornerRadius(wux::CornerRadiusHelper::FromUniformRadius(8));

            winrt::Windows::UI::Color fill{};
            fill.A = 235; fill.R = 220; fill.G = 40; fill.B = 40;
            wuxm::SolidColorBrush brush;
            brush.Color(fill);
            box.Background(brush);

            wuxc::TextBlock label;
            label.Text(L"Windhawk paint test");
            label.FontSize(16);
            ...
            Rec(L"SUCCESS: element appended to RootGrid. If a red box is "
                L"visible on the search page, XAML injection works in the "
                L"AppContainer.");
        } catch (...) {
            Rec(L"FAIL: constructing or appending XAML threw %08X",
...
void Wh_ModAfterInit() {
    Wh_Log(L">");
    Rec(L"=== paint test attached, pid=%lu ===", GetCurrentProcessId());
