// ==WindhawkMod==
// @id              widen2
// @name            Search widen test 2 (temporary)
...
Output goes to `%TEMP%\widen2.log`.
*/
// ==/WindhawkModReadme==
...
// Append a line to %TEMP%\widen2.log. The host owns the debugger channel
...
void Rec(const wchar_t* fmt, ...) {
    ...
    wcscat_s(path, MAX_PATH, L"widen2.log");
   ...
   private:
    HRESULT STDMETHODCALLTYPE OnVisualTreeChange(ParentChildRelation,
        ...
        *token = page.LayoutUpdated(
            winrt::auto_revoke,
            [pageRef, rootRef, wvRef, token](wf::IInspectable const&,
                                             wf::IInspectable const&) {
                if (g_injected) {
                    return;
                }
                double wvW = wvRef.ActualWidth();
                double wvH = wvRef.ActualHeight();
                if (wvW < 100 || wvH < 100) {
                    return;  // not laid out yet
                }

                HWND flyout = FindFlyoutWindow();
                if (!flyout) {
                    return;
                }
                RECT r{};
                GetWindowRect(flyout, &r);
                int oldW = r.right - r.left, oldH = r.bottom - r.top;

                Rec(L"layout ready: WebViewGrid %.0fx%.0f, flyout %dx%d",
                    wvW, wvH, oldW, oldH);

                try {
                    g_rootGrid = rootRef;
                    g_webViewGrid = wvRef;
                    g_originalMargin = wvRef.Margin();
                    g_flyout = flyout;
                    g_originalRect = r;

                    int newW = oldW + (int)kPanelWidth;
                    SetWindowPos(flyout, nullptr, 0, 0, newW, oldH,
                                 SWP_NOMOVE | SWP_NOZORDER | SWP_NOACTIVATE);
                    RECT after{};
                    GetWindowRect(flyout, &after);
                    ...
                    panel.Child(label);
                    rootRef.Children().Append(panel);
                    g_panel = panel;
                    g_injected = true;

                    Rec(L"applied: flyout %d -> %d, WebViewGrid margin.right "
                        L"%.0f -> %.0f, panel added",
                        oldW, after.right - after.left, g_originalMargin.Right,
                        shrunk.Right);
                } catch (...) {
...
void Wh_ModAfterInit() {
    Wh_Log(L">");
    Rec(L"=== widen2 attached, pid=%lu ===", GetCurrentProcessId());
