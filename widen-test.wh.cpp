// ==WindhawkMod==
// @id              widen-test
// @name            Search widen test (temporary)
// @description     Widens the search flyout and puts a panel in the new space, leaving the results area its original width.
// @version         0.1
// @author          bardelyne
...
Output goes to `%TEMP%\widen-test.log`.
*/
// ==/WindhawkModReadme==
...
// Append a line to %TEMP%\widen-test.log. The host owns the debugger channel
...
void Rec(const wchar_t* fmt, ...) {
    ...
    wcscat_s(path, MAX_PATH, L"widen-test.log");
...
HWND FindFlyoutWindow() {
    ...
    EnumWindows(
        [](HWND hwnd, LPARAM lp) -> BOOL {
            ...
            if (r.right - r.left > 400 && r.bottom - r.top > 400) {
                c->best = hwnd;
                return FALSE;
            }
            return TRUE;
   ...
   private:
    HRESULT STDMETHODCALLTYPE OnVisualTreeChange(ParentChildRelation,
        ...
        try {
            g_rootGrid = root;
            g_webViewGrid = wv;
            g_originalMargin = wv.Margin();

            // Push the web view's right edge in to free a column.
            // Grow the window first, then push the web view's right edge in
            // by the same amount. Net effect: the results area keeps the width
            // it already had, so it never reaches the responsive breakpoint
            // that collapsed its list, and the new space is ours.
            g_flyout = FindFlyoutWindow();
            if (!g_flyout) {
                Rec(L"FAIL: could not find the flyout window");
                return S_OK;
            }
            GetWindowRect(g_flyout, &g_originalRect);
            int oldW = g_originalRect.right - g_originalRect.left;
            int oldH = g_originalRect.bottom - g_originalRect.top;
            int newW = oldW + (int)kPanelWidth;
            BOOL ok = SetWindowPos(g_flyout, nullptr, 0, 0, newW, oldH,
                                   SWP_NOMOVE | SWP_NOZORDER | SWP_NOACTIVATE);
            RECT after{};
            GetWindowRect(g_flyout, &after);
            Rec(L"flyout width %d -> asked %d, got %d (SetWindowPos=%d)",
                oldW, newW, after.right - after.left, ok ? 1 : 0);

            wux::Thickness shrunk = g_originalMargin;
            shrunk.Right = g_originalMargin.Right + kPanelWidth;
            wv.Margin(shrunk);
            Rec(L"WebViewGrid margin.right %.0f -> %.0f", g_originalMargin.Right,
                shrunk.Right);

            wuxc::Border panel;
            panel.Name(L"WindhawkFilesPanel");
            panel.Width(kPanelWidth - 16);
            panel.HorizontalAlignment(wux::HorizontalAlignment::Right);
            panel.VerticalAlignment(wux::VerticalAlignment::Stretch);
            panel.Margin(wux::ThicknessHelper::FromLengths(0, 56, 8, 8));
            panel.CornerRadius(wux::CornerRadiusHelper::FromUniformRadius(8));

            winrt::Windows::UI::Color bg{};
            bg.A = 40; bg.R = 255; bg.G = 255; bg.B = 255;
            wuxm::SolidColorBrush brush;
            brush.Color(bg);
            panel.Background(brush);

            wuxc::TextBlock label;
            label.Text(L"Files  -  Everything results go here");
            label.FontSize(14);
            label.Margin(wux::ThicknessHelper::FromLengths(16, 16, 16, 16));
            label.HorizontalAlignment(wux::HorizontalAlignment::Center);
            label.VerticalAlignment(wux::VerticalAlignment::Top);
            panel.Child(label);

            root.Children().Append(panel);
            g_panel = panel;
            g_injected = true;
            Rec(L"SUCCESS: web view shrunk and panel added beside it");
        } catch (...) {
            Rec(L"FAIL: %08X", static_cast<unsigned>(winrt::to_hresult()));
        }
...
void Wh_ModAfterInit() {
    Wh_Log(L">");
    Rec(L"=== widen test attached, pid=%lu ===", GetCurrentProcessId());
...
void RemoveInjection() {
    try {
        if (g_webViewGrid) {
            g_webViewGrid.Margin(g_originalMargin);
            Rec(L"restored WebViewGrid margin");
        }
        if (g_flyout) {
